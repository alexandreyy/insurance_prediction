...
def reportBestResult():
     ...
     confusion_matrix_history = load_np_array("results/rfc_folds_confusion_"+str(c)+".bin")
...
#            mean_cm[i][j] = confusion_matrix_history[i][j].mean()
     ...
     recall_mean = []
     for c in C: 
         accuracy_history = load_np_array("results/ada_accuracy_"+str(c)+".bin")
...
if __name__ == '__main__':
    ...
    results_f = open("ada_results.csv", 'w')
...
#    homesite.train_y = homesite.train_y[reduced_range]    
    ...
    for c in C:  
        ...
        for i,(train, test) in enumerate(cvs):
            ...
            save_np_array("results/ada_accuracy_"+str(c)+".bin", np.array(accuracy_history))
            save_np_array("results/ada_precision_"+str(c)+".bin", np.array(precision_history))
            save_np_array("results/ada_recall_"+str(c)+".bin", np.array(recall_history))
            save_np_array("results/ada_auc_"+str(c)+".bin", np.array(auc_history))
...
#        print mean_precision
        ...
        save_np_array("results/rfc_folds_confusion_"+str(c)+".bin", np.array(confusion_matrix_history))
#        filename = 'D:\MACHINE_LEARNING\ML_FINAL_PROJECT\classifiers\adaboost_'+str(c)+'.joblib.pkl'
...
#    accuracy_history = load_np_array("results/ada_accuracy_128.bin")
#    precision_history = load_np_array("results/ada_precision_128.bin")
#    recall_history = load_np_array("results/ada_recall_128.bin")
#    auc_history = load_np_array("results/ada_auc_128.bin")
#    
#    
